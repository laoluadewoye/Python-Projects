def move(lists):  # Easiest as it's default
    ...
    if "o" in optionInput and "t" in optionInput:  # indexes needed for possibly both
        ...
        if "i" in optionInput:  # index needed. need option for just 'i' alone
            if optionInput == "-i":  # indexes only for some reason...
                ...
                print("Well theoretically it could, but why would you care about something sent to Null?")
            elif "o" in optionInput:  # original list included. o and i
                value = originalOption(lists, 1)
...
def optionsCheck(optint):
    ...
    # erasing options in separate variable
...
def moveOptionsMain(lists, optionInput):
    ...
    if isValid:  # If true, goes into the options tree to find the proper output
